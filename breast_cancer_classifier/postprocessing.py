...
def main():
    if not os.path.exists('./breast_cancer_detector.pickle'): 
        print('Pickled model cannot be found. Lets train it!') 
        training_main()
    
    with open('./data/preprocessed_data.pickle', 'rb') as f:
        _, X_test_scaled, _, y_test = pickle.load(f)

    with open('./breast_cancer_detector.pickle', 'rb') as f:
        best_xgb_classifier = pickle.load(f)

    print("Pickled model and preprocessed data was read. Lets postprocess it!")
    ...
    with open('./data/test_predicted.txt', 'wt') as f:
        f.write("\n".join(map(str, y_pred)) + '\n')
        print('Save predicted labels to ./data/test_predicted.txt')
    ...
    with open('data/classification_report.txt', 'w') as f:
        f.write(report)
        print('Save classification report to ./data/classification_report.txt')
    ...
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig('data/confusion_matrix.png')  # Save confusion matrix plot
    plt.close()  # Close the plot to avoid display in notebooks
    print('Save confusion matrix visialization to ./data/confusion_matrix.png')
